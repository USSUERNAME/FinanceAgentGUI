...
The empty grid-cell control has a mode-specific contract. In `strategy-research`, it opens a prompt modal and the sidebar agent may create a typed widget graph. In `asset-management`, it opens the local widget picker and the user directly selects the snapshot-backed widget to add. The asset-management sidebar agent must not emit widget-creation actions even when asked; it should explain that this canvas has a simpler widget structure than the strategy canvas and direct the user to the empty cell's `+` button.
...
The asset-management sidebar agent is an explainer and diagnostic guide, not a widget creator. `create_widget`, `create_widget_flow`, `create_portfolio_widget`, and `render_portfolio_artifact` are rejected in this mode. The agent may recommend which picker item fits the request, but the user creates it through the canvas `+` button. Explicitly targeted existing-widget updates or deletes may still use their normal actions; a missing update target must never fall back to creating a new widget. Strategy-research agent creation remains unchanged.
...
Asset-management empty cells use a local widget picker instead of an agent prompt. The picker includes `보유 자산 과거 내역 차트`, which creates a `visualType:"price-history"` widget. That widget reads `/api/tossinvest/order-sync/investment-history`, uses the visible asset-history period and timeframe, and renders the returned snapshot valuation series with Lightweight Charts. When comparison assets are committed in `chartSpec.query.comparisonAssets`, the same widget switches to a normalized start-100 line comparison: the user's portfolio is the thick line and up to five comparison assets are thin colored lines. It is not a yfinance backtest widget and should not receive `run_backtest_chart_widget`.

The `평가 테이블` button on a `price-history` widget toggles one separate dependent `metrics-table` with `chartSpec.role:"asset_history_evaluation"`, `dependsOn:[sourceWidgetId]`, and `updatePolicy:"auto"`. Pressing the inactive button creates the table; pressing the active filled button again removes that connected table immediately. The table reads the source widget's current query at render time rather than copying metric rows into storage. It therefore recomputes Cumulative Return, CAGR, MDD, Volatility, Sharpe, Sortino, Calmar, Ulcer, UPI, and BETA whenever the visible date range, timeframe, currency, or comparison-asset list changes. The first row is always `보유자산`; each committed comparison asset adds one row. BETA is never measured against the holdings series itself: the table header owns a `BETA: VOO | KODEX 200` switch, defaults to VOO, uses symbol `069500` when KODEX 200 is selected, persists the selection in `chartSpec.betaBenchmark`, first aligns portfolio and benchmark prices to dates shared by both series, and then calculates covariance divided by benchmark variance from the resulting common-interval returns. Deleting the source `price-history` widget must cascade-delete this role-specific dependent table so it cannot remain orphaned. While the dependent exists, the source widget's `평가 테이블` button stays in its active filled state.

Asset-management widgets that fetch data at render time publish a `portfolio-widget-display-data.v1` snapshot back to the current canvas Context Packet. `displayData.summary` contains the compact values the sidebar agent should always know, while `displayData.data` carries the complete currently displayed rows or series only to the local chat request. The server builds a request-scoped `query-scoped-local-rag` chunk index and injects only the chunks relevant to the user's current question. These snapshots are not appended to shared memory, are not written to disk, and must remain scoped to the current canvas.
...
- Local user-owned import flows may create an allocation widget from explicit holdings data such as `AAPL 60%, MSFT 40%` or rows with `평가금액`, `평가액`, `marketValue`, `market_value`, `amount`, `비중`, or `weight` columns. This is not a sidebar-agent widget-creation path.
...
- In asset-management mode, when a broker-backed local flow creates a ready holdings `table`, the GUI should use the same automatic derived pie chart path.
...
- Asset-management empty-cell creation must stay in the local `+` picker. It must not open a prompt-only modal or forward widget-creation text to the sidebar agent.
