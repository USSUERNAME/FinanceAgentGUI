...
- The external memory layer uses the latest World Memory report summary with `월드 메모리 변경 제안` removed, plus a translation-model market summary of News Feed items after `data/world-memory/collector-state.json` `collector.lastSuccessfulAt`. The same model response must include semantic severity fields (`alertLevel`, `severityKo`, `shouldCreateReport`, `pushSummary`) so severity is generated with the summary instead of by a second text-matching pass. The briefing is refreshed in place every 15 minutes rather than accumulated as an endless digest log, raw News Feed item lists should not be appended to prompt memory, and `/api/memory` runs the existing urgent-market-update procedure once when the summary is `urgent` or `critical`. Exact News Feed + sanitized World Memory + provider/model/reasoning + prompt-version SHA-256 matches must reuse the successful persisted summary without an LLM call; concurrent refreshes share one runtime-file-lease single flight, and prompts keep stable instructions/schema plus the World Memory baseline before the changing News Feed rows so provider prefix caching remains available.
...
- Reader-facing magazine prose should follow the v2 article writing harness in `docs/magazine.md`, `config/magazine-article-style-v2.prompt.md`, and `config/magazine-longform-editorial-standard.prompt.md`: no exposed retrieval-process phrases, no editorial checklist placeholders in article bodies, no fixed `H2 + two paragraphs` rhythm, no scolding/teacherly repeated advice, and no pervasive translated-English structure built from abstract personification, noun-phrase chains, omitted semantic relations, or manufactured aphorisms. The default Codex path uses one isolated structured writer turn with three approved prose exemplars and the semantically selected evidence. A whole-article naturalness or evidence failure stops the cycle without an LLM repair loop. Source-backed direct/indirect quote attribution is required when the selected evidence contains material stakeholder comments.
...
- Run `node scripts/magazine_article_quality_check.mjs --strict` before publishing v2 generated magazine articles. The integrated one-shot editorial review, verified image, cover classifier, and event-signature checks remain publication gates.
